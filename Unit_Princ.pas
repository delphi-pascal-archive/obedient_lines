...
procedure TFichePrinc.Redessiner;
  ...
  with Img_Surface.Canvas do begin
    ...
    if Selection in [LePoint1, LePoint2, LaLigne] then begin
      ...
      {Dessin de l'ombre des poignées de la ligne sélectionnée.}
    ...
    if Selection in [LePoint1, LePoint2, LaLigne] then begin
      ...
      MoveTo(Pt1.X, Pt1.Y);
      LineTo(Pt2.X, Pt2.Y);
      {Dessin des poignées de la ligne sélectionnée.}
      Brush.Color := clLime;    //Couleur intérieure.
      Pen.Color   := clNavy;    //Couleur des bords.
      Ellipse(Pt1.X-Ecart,Pt1.Y-Ecart,Pt1.X+Ecart,Pt1.Y+Ecart);
      Ellipse(Pt2.X-Ecart,Pt2.Y-Ecart,Pt2.X+Ecart,Pt2.Y+Ecart);
    end;
  end;//with
end;
